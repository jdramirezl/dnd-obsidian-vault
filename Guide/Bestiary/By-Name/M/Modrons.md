---
type: monster
source: Monster Manual
cr: 2
creature_type: construct
size: Large
alignment: lawful neutral
has_legendary: false
has_lair: false
grouped: true
tags:
  - monster
  - monster-manual
  - construct
  - cr/2
---
# Modrons

![Modrons](https://www.aidedd.org/dnd/images/monodrone.jpg)


> **Type**: Medium/Large construct, lawful neutral | **CR**: 1/8 – 2 | **Source**: Monster Manual p.224–227

Modrons are beings of absolute law that adhere to a hive-like hierarchy. They inhabit the plane of Mechanus and tend its eternally revolving gears, their existence a clockwork routine of perfect order. Under the direction of their leader, Primus, modrons increase order in the multiverse in accordance with laws beyond the comprehension of mortal minds.

---

## Monodrone

> **Type**: Medium construct, lawful neutral | **CR**: 1/8 (25 XP) | **Source**: Monster Manual p.224

A monodrone can perform one simple task at a time and can relay a single message of up to forty-eight words.

### Stat Block

**Armor Class** 15 (natural armor)
**Hit Points** 5 (1d8 + 1)
**Speed** 30 ft., fly 30 ft.

| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| 10 (+0) | 13 (+1) | 12 (+1) | 4 (-3) | 10 (+0) | 5 (-3) |

**Senses** truesight 120 ft., passive Perception 10
**Languages** Modron
**Challenge** 1/8 (25 XP)

#### Traits

**Axiomatic Mind.** The monodrone can't be compelled to act in a manner contrary to its nature or its instructions.

**Disintegration.** If the monodrone dies, its body disintegrates into dust, leaving behind its weapons and anything else it was carrying.

#### Actions

**Dagger.** *Melee Weapon Attack:* +3 to hit, reach 5 ft., one target. *Hit:* 3 (1d4 + 1) piercing damage.

**Javelin.** *Melee or Ranged Weapon Attack:* +3 to hit, reach 5 ft. or range 30/120 ft., one target. *Hit:* 4 (1d6 + 1) piercing damage.

---

## Duodrone

> **Type**: Medium construct, lawful neutral | **CR**: 1/4 (50 XP) | **Source**: Monster Manual p.225

The blocky duodrones supervise units of monodrones and can perform up to two tasks at a time.

### Stat Block

**Armor Class** 15 (natural armor)
**Hit Points** 11 (2d8 + 2)
**Speed** 30 ft.

| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| 11 (+0) | 13 (+1) | 12 (+1) | 6 (-2) | 10 (+0) | 7 (-2) |

**Senses** truesight 120 ft., passive Perception 10
**Languages** Modron
**Challenge** 1/4 (50 XP)

#### Traits

**Axiomatic Mind.** The duodrone can't be compelled to act in a manner contrary to its nature or its instructions.

**Disintegration.** If the duodrone dies, its body disintegrates into dust, leaving behind its weapons and anything else it was carrying.

#### Actions

**Multiattack.** The duodrone makes two fist attacks or two javelin attacks.

**Fist.** *Melee Weapon Attack:* +2 to hit, reach 5 ft., one target. *Hit:* 2 (1d4) bludgeoning damage.

**Javelin.** *Melee or Ranged Weapon Attack:* +3 to hit, reach 5 ft. or range 30/120 ft., one target. *Hit:* 4 (1d6 + 1) piercing damage.

---

## Tridrone

> **Type**: Medium construct, lawful neutral | **CR**: 1/2 (100 XP) | **Source**: Monster Manual p.225

Tridrones are shaped like inverted pyramids. They lead lesser modrons in battle.

### Stat Block

**Armor Class** 15 (natural armor)
**Hit Points** 16 (3d8 + 3)
**Speed** 30 ft.

| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| 12 (+1) | 13 (+1) | 12 (+1) | 9 (-1) | 10 (+0) | 9 (-1) |

**Senses** truesight 120 ft., passive Perception 10
**Languages** Modron
**Challenge** 1/2 (100 XP)

#### Traits

**Axiomatic Mind.** The tridrone can't be compelled to act in a manner contrary to its nature or its instructions.

**Disintegration.** If the tridrone dies, its body disintegrates into dust, leaving behind its weapons and anything else it was carrying.

#### Actions

**Multiattack.** The tridrone makes three fist attacks or three javelin attacks.

**Fist.** *Melee Weapon Attack:* +3 to hit, reach 5 ft., one target. *Hit:* 3 (1d4 + 1) bludgeoning damage.

**Javelin.** *Melee or Ranged Weapon Attack:* +3 to hit, reach 5 ft. or range 30/120 ft., one target. *Hit:* 4 (1d6 + 1) piercing damage.

---

## Quadrone

> **Type**: Medium construct, lawful neutral | **CR**: 1 (200 XP) | **Source**: Monster Manual p.226

Astute combatants, quadrones serve as artillery and field officers in the regiments of modron armies.

### Stat Block

**Armor Class** 16 (natural armor)
**Hit Points** 22 (4d8 + 4)
**Speed** 30 ft., fly 30 ft.

| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| 12 (+1) | 14 (+2) | 12 (+1) | 10 (+0) | 10 (+0) | 11 (+0) |

**Skills** Perception +2
**Senses** truesight 120 ft., passive Perception 12
**Languages** Modron
**Challenge** 1 (200 XP)

#### Traits

**Axiomatic Mind.** The quadrone can't be compelled to act in a manner contrary to its nature or its instructions.

**Disintegration.** If the quadrone dies, its body disintegrates into dust, leaving behind its weapons and anything else it was carrying.

#### Actions

**Multiattack.** The quadrone makes two fist attacks or four shortbow attacks.

**Fist.** *Melee Weapon Attack:* +3 to hit, reach 5 ft., one target. *Hit:* 3 (1d4 + 1) bludgeoning damage.

**Shortbow.** *Ranged Weapon Attack:* +4 to hit, range 80/320 ft., one target. *Hit:* 5 (1d6 + 2) piercing damage.

---

## Pentadrone

> **Type**: Large construct, lawful neutral | **CR**: 2 (450 XP) | **Source**: Monster Manual p.226

Pentadrones oversee Mechanus's worker populace and can improvise in response to new situations.

### Stat Block

**Armor Class** 16 (natural armor)
**Hit Points** 32 (5d10 + 5)
**Speed** 40 ft.

| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| 15 (+2) | 14 (+2) | 12 (+1) | 10 (+0) | 10 (+0) | 13 (+1) |

**Skills** Perception +4
**Senses** truesight 120 ft., passive Perception 14
**Languages** Modron
**Challenge** 2 (450 XP)

#### Traits

**Axiomatic Mind.** The pentadrone can't be compelled to act in a manner contrary to its nature or its instructions.

**Disintegration.** If the pentadrone dies, its body disintegrates into dust, leaving behind its weapons and anything else it was carrying.

#### Actions

**Multiattack.** The pentadrone makes five arm attacks.

**Arm.** *Melee Weapon Attack:* +4 to hit, reach 5 ft., one target. *Hit:* 5 (1d6 + 2) bludgeoning damage.

**Paralysis Gas (Recharge 5–6).** The pentadrone exhales a 30-foot cone of gas. Each creature in that area must succeed on a DC 11 Constitution saving throw or be [[Paralyzed|paralyzed]] for 1 minute. A creature can repeat the saving throw at the end of each of its turns, ending the effect on itself on a success.

---

## Variant: Rogue Modrons

A modron unit sometimes becomes defective, either through natural decay or exposure to chaotic forces. Rogue modrons don't act in accordance with Primus's wishes and directives, breaking laws, disobeying orders, and even engaging in violence. Other modrons hunt down such rogues.

A rogue modron loses the **Axiomatic Mind** trait and can have any alignment other than lawful neutral. Otherwise, it has the same statistics as a regular modron of its rank.

---

## Lore

**Absolute Law and Order.** Modrons' minds are networked in a hierarchical pyramid, in which each modron receives commands from superiors and delegates orders to underlings. A modron carries out commands with total obedience, utmost efficiency, and an absence of morality or ego. To a modron, there is no "I," but only "we" or "us."

**Absolute Hierarchy.** Modrons communicate only with their own rank and the ranks immediately above and below them. Modrons more than one rank away are either too advanced or too simple to understand.

**Cogs of the Great Machine.** If a modron is destroyed, its remains disintegrate. A replacement from the next lowest rank then transforms in a flash of light, gaining the physical form of its new rank. The promoted modron is replaced by one of its underlings in the same manner, all the way to the lowest levels of the hierarchy.

**The Great Modron March.** When the gears of Mechanus complete seventeen cycles once every 289 years, Primus sends a vast army of modrons across the Outer Planes, ostensibly on a reconnaissance mission. The march is long and dangerous, and only a small number of modrons returns to Mechanus.

---

## DM Quick Reference

> [!quote] Tactical Deep Dive
> See [[Modron-Tactics]] for detailed combat tactics from *The Monsters Know What They're Doing*.


> [!tip] Truesight 120 ft. on Everything
> Every modron has truesight 120 ft. — invisibility, illusions, disguises, and shapechangers are all useless against them. This makes modron patrols uniquely dangerous for parties that rely on stealth and deception.

> [!example] Rogue Modrons
> A rogue modron loses Axiomatic Mind and can have any non-lawful-neutral alignment. These make excellent quirky NPCs — a chaotic good pentadrone seeking purpose, or a neutral evil quadrone gone mercenary. Same stats, completely different personality.

- **What they are:** Clockwork constructs of pure law from Mechanus. Five ranks from Monodrone (CR 1/8) to Pentadrone (CR 2). Each rank can perform one more task than the last. All share Axiomatic Mind (immune to compulsion) and Disintegration (no corpse).
- **When to use:** Planar adventures, Mechanus encounters, or as bizarre visitors to the Material Plane during the Great Modron March. Rogue modrons make interesting NPCs or allies.
- **Key combat tactics:** Modrons fight in organized formations. Number of attacks = rank number (Monodrone 1, Duodrone 2, etc.). Pentadrone has Paralysis Gas (30 ft. cone, DC 11 Con, Recharge 5–6). Quadrones provide ranged support with shortbows (4 attacks). All have truesight 120 ft. — invisibility and illusions are useless.
- **Quick comparison:**
  - *Monodrone (CR 1/8):* 5 HP, 1 attack, fly 30 ft.
  - *Duodrone (CR 1/4):* 11 HP, 2 attacks, no fly
  - *Tridrone (CR 1/2):* 16 HP, 3 attacks, no fly
  - *Quadrone (CR 1):* 22 HP, 2 fists or 4 shortbow, fly 30 ft.
  - *Pentadrone (CR 2):* 32 HP, 5 arms + Paralysis Gas, Large, 40 ft. speed
- **Conditions inflicted:** [[Paralyzed|paralyzed]] (Pentadrone Paralysis Gas)
- **Suggested encounter pairings:** A patrol of mixed ranks (1 pentadrone, 2 quadrones, 4 tridrones, etc.). A rogue modron seeking help. Modron march passing through a town. Pair with inevitables or other Mechanus creatures.
